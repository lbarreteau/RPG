...
{
    screen->window = sfRenderWindow_create(screen->mode, screen->name_screen, sfResize | sfClose,
    NULL);
    sfRenderWindow_setFramerateLimit(screen->window, screen->fps);
    screen->background = init_sprite(screen->name_background, (sfVector2f) {2.5, 2.5}, (sfVector2f) {0, 0});
}
